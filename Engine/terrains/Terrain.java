...
public class Terrain {
	...
	//private static final int SEED = 2138765349;
	private static final int SEED = 2138165569;
	...
	private RawModel generateTerrain(Loader loader, String heightMap) {
		...
		try {
			image = ImageIO.read(getClass().getClassLoader().getResource("res/" + heightMap + ".png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		for (int i = 0; i < VERTEX_COUNT; i++) {
			for (int j = 0; j < VERTEX_COUNT; j++) {
				...
				float height = getHeight(j, i, generator); // image: heightmap, generator: random
