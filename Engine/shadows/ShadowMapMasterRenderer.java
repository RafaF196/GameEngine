...
public class ShadowMapMasterRenderer {
	...
	public ShadowMapMasterRenderer(Camera camera) {
		...
		terShader = new TerrainShadowShader();
		aniShader = new AnimatedShadowShader();
		shadowBox = new ShadowBox(lightViewMatrix, camera);
		shadowFbo = new ShadowFrameBuffer(SHADOW_MAP_SIZE, SHADOW_MAP_SIZE);
		entityRenderer = new ShadowMapEntityRenderer(shader, terShader, aniShader, projectionViewMatrix);
	}

	public void render(Map<TexturedModel, List<Entity>> entities, Map<TexturedModel, List<Entity>> normalEntities,
			List<Terrain> terrains, Map<AnimatedModel, List<AnimatedEntity>> animatedEntities, Light sun) {
		shadowBox.update();
		Vector3f sunPosition = sun.getPosition();
		Vector3f lightDirection = new Vector3f(-sunPosition.x, -sunPosition.y, -sunPosition.z);
		prepare(lightDirection, shadowBox);
		entityRenderer.render(entities, normalEntities, terrains, animatedEntities);
		finish();
	}
